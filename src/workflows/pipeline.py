...
class Pipeline:
    ...
    async def run_generation(self, summary_mode: bool = False):
        ...
        # Mix sources: Take top 30 from each source type
        mixed_candidates = []
        ...
        BATCH_SIZE = 8 # Safe for local LLM context
        
        # Process in batches
        for i in range(0, len(candidates), BATCH_SIZE):
            batch = candidates[i : i + BATCH_SIZE]
            
            # For each active persona, evaluate this batch
            for persona_name in active_personas:
                try:
                    logger.info(f"Batch Evaluating {len(batch)} items for [{persona_name}]...")
                    ...
                    for res in results:
                        ...
                        if item and res.decision == "KEEP":
                            # Strict Check: Score > 5
                            if res.score > 5:
                                info_tuple = (item, res)
                                if persona_name == "GENAI_NEWS": accepted_genai.append(info_tuple)
                                elif persona_name == "PRODUCT_IDEAS": accepted_prod.append(info_tuple)
                                elif persona_name == "FINANCIAL_ANALYSIS": accepted_finance.append(info_tuple)
                                logger.info(f"KEEP [{persona_name}] {item.title} (Score: {res.score})")
                except Exception as e:
                    logger.error(f"Batch evaluation failed for {persona_name}: {e}")

        # 3. Summarize & Format
        # Strictly Limit to Top 5 per category
    ...
    def format_digest(self, 
        ...
        if genai:
            ...
            for item, res in genai:
                ...
                lines.append(f"**Source:** {item.source} | **Relevance:** {int(res.score)}/10")
        ...
        if prod:
            ...
            for item, res in prod:
                lines.append(f"### [{item.title}]({item.url})")
                lines.append(f"**Source:** {item.source} | **Score:** {int(res.score)}/10")
                lines.append(f"**Insight:** {res.reasoning}")
                lines.append("")
